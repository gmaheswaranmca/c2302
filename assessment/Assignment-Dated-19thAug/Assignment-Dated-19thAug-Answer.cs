...
class MyArray
{
	...
	public static bool IsDivisibleBy5(int first)
	{
		return first % 5==0;
	}
	
	public static void FindMaxMinDiffIs5Divisible(int[] dataList, int startIndex, int endIndex,
	...
	public static void SortAgeByFreqDesc(int[] dataList, int startIndex, int endIndex, int[] freqList)
	{
		for (int i = startIndex; i <= (endIndex - 1); i++)
		{
			for (int j = i + 1; j <= endIndex; j++)
			{
				...
				{	
					Swap(ref dataList[j], ref dataList[i]);
					Swap(ref freqList[j], ref freqList[i]);
				}
			}
		}
	}
...
class Coworker
{
	...
	public static void DoTask_1_v1(int[] ages,int size)//5Divisibility(Diff((Max and Min)))
	{
		...
		bool isDivisible = MyArray.IsDivisibleBy5(diff);
		...
		if(isDivisible)
			Console.WriteLine("the diff is divisible by 5");
		else 
			Console.WriteLine("the diff is not divisible by 5");
	}
	public static void DoTask_1_v2(int[] ages,int size)//5Divisibility(Diff((Max and Min)))
	{
		...
		MyArray.FindMaxMinDiffIs5Divisible(ages, 0, size - 1,
